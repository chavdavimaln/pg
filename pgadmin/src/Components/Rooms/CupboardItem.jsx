...
const CupboardItem = ({ item, onDrag, selected, onSelect }) => {
    ...
    return (
        <Draggable nodeRef={nodeRef} bounds="parent" defaultPosition={{ x: item.x, y: item.y }} onStop={(e, data) => onDrag(item.id, data.x, data.y)}>
            <div ref={nodeRef} onClick={onSelect} className={`absolute w-13 h-9 bg-green-700 text-white text-[11px] rounded-lg flex items-center justify-center cursor-move ${selected ? "border-2 border-red-500" : "border-green-800"}`}>
                {item.label}
            </div>
        </Draggable>
    );
